...
void FRToSI2C::FRToSInit() {
	if (this->_I2CSemaphore == nullptr)
	{
		//this->_I2CSemaphore = xSemaphoreCreateBinaryStatic(&(this->_xSemaphoreBuffer));
		this->_I2CSemaphore = xSemaphoreCreateMutexStatic(&(this->_xSemaphoreBuffer));
		unlock();//同xSemaphoreGive(this->_I2CSemaphore);//释放信号量
	}
	if (this->_I2CSemaphore2 == nullptr)
	{
		this->_I2CSemaphore2 = xSemaphoreCreateMutexStatic(&(this->_xSemaphoreBuffer2));
		unlock2();//同xSemaphoreGive(this->_I2CSemaphore);//释放信号量
	}
}
...
void FRToSI2C::CpltCallback(I2C_HandleTypeDef *I2C_Handle) {
  ...
  if((I2C_Handle == FRToSI2C1.getI2C_Handle()) && (*FRToSI2C1.getI2CSemaphore()))
	  xSemaphoreGiveFromISR(*FRToSI2C1.getI2CSemaphore(), NULL);

  else if((I2C_Handle == FRToSI2C2.getI2C_Handle()) && (*FRToSI2C2.getI2CSemaphore()))
...
bool FRToSI2C::Mem_Read(uint16_t DevAddress, uint16_t MemAddress, uint8_t *pData, uint16_t Size) {
  ...
  if (HAL_I2C_Mem_Read(this->_I2C_Handle, DevAddress, MemAddress, I2C_MEMADD_SIZE_8BIT, pData, Size, 500) != HAL_OK) {
...
bool FRToSI2C::Mem_Write(uint16_t DevAddress, uint16_t MemAddress, uint8_t *pData, uint16_t Size) {
  ...
  if (!lock())
    return false;
  if (HAL_I2C_Mem_Write(this->_I2C_Handle, DevAddress, MemAddress, I2C_MEMADD_SIZE_8BIT, pData, Size, 500) != HAL_OK) {
...
bool FRToSI2C::Transmit(uint16_t DevAddress, uint8_t *pData, uint16_t Size) {
  ...
  if (HAL_I2C_Master_Transmit_DMA(this->_I2C_Handle, DevAddress, pData, Size) != HAL_OK) {
    I2C_Unstick();
    unlock();
...
bool    FRToSI2C::Master_Transmit(uint16_t DevAddress, uint8_t *pData, uint16_t Size)
{
  if (!lock())
  {
	  usb_printf("locked! Can't Transmit\r\n");
  ...
  if (HAL_I2C_Master_Transmit(this->_I2C_Handle, DevAddress, pData, Size,  HAL_MAX_DELAY) != HAL_OK) {
	I2C_Unstick();
	unlock();
	usb_printf("!= HAL_OK, Can't Transmit\r\n");
	return false;
...
bool    FRToSI2C::Master_Receive(uint16_t DevAddress, uint8_t *pData, uint16_t Size)
{
	  if (!lock())
	  {
		  usb_printf("locked! Can't Transmit\r\n");
	  ...
	  if (HAL_I2C_Master_Receive(this->_I2C_Handle, DevAddress, pData, Size,  HAL_MAX_DELAY) != HAL_OK) {
		I2C_Unstick();
		unlock();
		usb_printf("!= HAL_OK, Can't Transmit\r\n");
		return false;
...
bool FRToSI2C::probe(uint16_t DevAddress) {
  ...
  uint8_t buffer[1];
  bool    worked = HAL_I2C_Mem_Read(this->_I2C_Handle, DevAddress, 0x0F, I2C_MEMADD_SIZE_8BIT, buffer, 1, 1000) == HAL_OK;
  unlock();
  return worked;
}
...
void FRToSI2C::I2C_Unstick() {
	unstick_I2C(this->_I2C_Handle);
}


//释放_I2CSemaphore (前提FRToI2C::lock成功获取_I2CSemaphore)
void FRToSI2C::unlock() {
	xSemaphoreGive(this->_I2CSemaphore);
}
...
bool FRToSI2C::lock() {
	bool mark = 0;
	mark = (xSemaphoreTake(this->_I2CSemaphore, (TickType_t)50) == pdTRUE);
	return mark;
}
