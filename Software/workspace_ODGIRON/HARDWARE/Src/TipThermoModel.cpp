...
uint32_t TipThermoModel::convertTipRawADCToDegC(uint16_t rawADC) {
	uint32_t val = convertTipRawADCTouV(rawADC);
	return convertuVToDegC(val);
}
uint32_t TipThermoModel::convertTipRawADCToDegF(uint16_t rawADC) {
	uint32_t val = convertTipRawADCTouV(rawADC);
	return convertuVToDegF(val);
}
...
uint32_t TipThermoModel::getTipMaxInC() {
  ...
  maximumTipTemp += getHandleTemperature(false) / 10;                                      // Add handle offset
