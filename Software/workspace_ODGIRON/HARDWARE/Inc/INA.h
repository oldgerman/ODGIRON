...
typedef struct {
	...
	uint32_t microOhmR :10;//默认是20;     ///< 0-1,048,575 Store initialization value
...
enum ina_Type {
	INA226,
#if 1 //ndef __INA226_ONLY
	INA219, INA230, INA231, INA260, INA3221_0, INA3221_1, INA3221_2,
#endif
	INA_UNKNOWN
};
...
public:
	...
	void setAveraging(const uint16_t averages, const uint8_t deviceNumber =
			UINT8_MAX);
	void setBusConversion(const uint32_t convTime, const uint8_t deviceNumber =
			UINT8_MAX);
	void setShuntConversion(const uint32_t convTime,
			const uint8_t deviceNumber = UINT8_MAX);
	...
	static uint8_t getDeviceCount() { return _DeviceCount;};
	...
	bool alertOnConversion(const bool alertState, const uint8_t deviceNumber =
			UINT8_MAX);
	bool alertOnShuntOverVoltage(const bool alertState,
			const int32_t milliVolts, const uint8_t deviceNumber = UINT8_MAX);
	bool alertOnShuntUnderVoltage(const bool alertState,
			const int32_t milliVolts, const uint8_t deviceNumber = UINT8_MAX);
...
private:
	...
	void writeWord(const uint8_t addr, const uint16_t data,
	...
	static uint8_t _DeviceCount;         ///< Total number of devices detected
	...
	//#else
	inaEEPROM _EEPROMEmulation[32]; //RAM模拟EEPROM///< Actual array of up to 32 devices
	//#endif
